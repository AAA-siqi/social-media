...
function useNotifConfig() {
  const t = useTranslations("notification")
  return {
    like: { icon: Heart, color: "text-red-500", bgColor: "bg-red-500/10", text: t("like") },
    comment: { icon: MessageCircle, color: "text-primary", bgColor: "bg-primary/10", text: t("comment") },
    follow: { icon: UserPlus, color: "text-primary", bgColor: "bg-primary/10", text: t("follow") },
    repost: { icon: Repeat2, color: "text-emerald-600", bgColor: "bg-emerald-600/10", text: t("repost") },
  }
}
...
export default function NotificationsPage() {
  ...
  const notifConfig = useNotifConfig()
    ...
    <AppShell>
      <div className="glass-card overflow-hidden">
        <header className="flex h-12 items-center border-b border-border/30 px-5">
          <h1 className="text-base font-bold text-card-foreground">{t("notification.title")}</h1>
        </header>
            ...
            <div key={notif.id}>
              <div className={cn("flex gap-3 px-5 py-3 transition-colors hover:bg-muted/20", !notif.read && "bg-primary/5")}>
                ...
                <div className="flex flex-1 flex-col gap-1">
                  ...
                  <time className="text-[11px] text-muted-foreground">{formatTime(notif.createdAt, locale)}</time>
                </div>
              </div>
